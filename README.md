...
| 13 | **P1 — 时区转换** | 可选开关 + IANA 目标时区（保存时校验）：改写 `<environment_context>` 块内的 `<timezone>` / `<current_date>`（支持原始与 HTML 转义两种方言），外加防御性的 `"timezone"` / `"timezone_offset_min"`；逐字节改写，无标记即原样返回；空或非法目标不转换 | 可让模型认为调用方在另一个时区 | 🔄 |
