...
func (s *ProxyStore) ListProxies(ctx context.Context) ([]proxies.Node, error) {
	rows, err := s.db.sql.QueryContext(ctx, `
		SELECT id, url, enabled, success_count, failure_count, last_latency_ms,
	...
	for rows.Next() {
		var (
			n                          proxies.Node
			enabled                    int
			lastUsed, lastOK, lastFail string
			latency                    sql.NullInt64
		)
		if err := rows.Scan(&n.ID, &n.URL, &enabled, &n.SuccessCount, &n.FailureCount,
			&latency, &lastUsed, &lastOK, &lastFail); err != nil {
	...
	return s.db.Write(ctx, func(tx *sql.Tx) error {
		_, err := tx.ExecContext(ctx, `
			INSERT INTO proxy_node
				(id, url, enabled, success_count, failure_count,
				 last_latency_ms, last_used_at, last_success, last_failure)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
			...
			intPtr(n.LastLatencyMS), FormatTimePtr(n.LastUsedAt),
			FormatTimePtr(n.LastSuccess), FormatTimePtr(n.LastFailure))
		if err != nil {
